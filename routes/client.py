    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            if (answer["POSITION_EMPLOYEE"] == "Secretaria"):
                sql = "SELECT CONCAT(e.NAME_EMPLOYEE,' ',e.LASTNAME_EMPLOYEE) as \"employee\" ,sc.ID_CLIENT ,CONCAT(c.NAME_CLIENT ,' ',c.LASTNAME_CLIENT) as \"client\", sc.ID_STAGE , s.NAME_STAGE ,DATE(sc.STAGE_END_DATE) AS \"STAGE_END_DATE\" ,sc.MEETING_TIME FROM STAGE_CLIENT sc,STAGE s, CLIENT c ,EMPLOYEE e  ,SUBSCRIBE sb WHERE CONDITIONS = 0 AND s.ID_STAGE =sc.ID_STAGE AND sc.ID_CLIENT =c.ID_CLIENT AND sb.ID_EMPLOYEE = e.ID_EMPLOYEE AND sb.ID_CLIENT  = c.ID_CLIENT GROUP BY sc.ID_CLIENT ORDER BY sc.STAGE_END_DATE LIMIT 10"
                cursor.execute(sql)
            else:
